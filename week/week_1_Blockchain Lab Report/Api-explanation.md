...
```json
{
  "bitcoin": {
    ...
    "usd_24h_change": 0.19,
    "krw": 98302614,
    "krw_market_cap": 1967825444714736,
    "krw_24h_change": 1.36
  },
  "ethereum": {
    "usd": 1948.65,
    ...
    "usd_24h_change": -0.63,
    "krw": 2874277,
    "krw_market_cap": 347378133597021,
    "krw_24h_change": 0.53
  }
}
```
...
| 필드 | 의미 | 예시 |
|------|------|------|
| `usd` | 미국 달러 기준 현재 가격 | 66645 ($66,645) |
| `krw` | 한국 원 기준 현재 가격 | 98302614 (₩98,302,614) |
| `usd_24h_change` | 24시간 동안 USD 기준 가격 변동률 (%) | 0.19 (+0.19%) |
| `usd_market_cap` | USD 기준 시가총액 | 약 $1.33T (1조 3천억 달러) |
...
서버가 "요청이 너무 많다"며 응답을 거부한다.
이후 일정 시간(보통 1분)이 지나면 다시 허용 → **됐다 안됐다 반복**
...
| **서버 사이드 호출** | Next.js Route Handler에서 호출 | **crypto-price-app에서 사용** |

- `crypto-prices.html` → CORS 프록시 3개를 폴백으로 사용
- `crypto-price-app` → Next.js의 Route Handler(`/api/prices`)가 서버에서 직접 호출하므로 CORS 문제 없음
