// ============================================================
//  Blockchain Lab #3 — GIWA Sepolia / Base Sepolia 테스트넷 실습 서버
...
const { NETWORKS, getNetwork, getAllNetworks } = require('./networks');
const NET = getNetwork(); // .env 기본 네트워크

const PORT = 5500;
const ALLOWED_ORIGIN = 'http://localhost:' + PORT;

// 모든 네트워크의 Provider 초기화 (프론트엔드 스위칭 지원)
const providers = {};
const wallets = {};
Object.keys(NETWORKS).forEach(function (key) {
  providers[key] = new ethers.JsonRpcProvider(NETWORKS[key].rpcUrl);
  if (process.env.PRIVATE_KEY) {
    wallets[key] = new ethers.Wallet(process.env.PRIVATE_KEY, providers[key]);
  }
});

// 기본 provider/wallet (.env 설정 기준)
const defaultNetKey = (process.env.NETWORK || 'giwa').toLowerCase();

// 요청에서 네트워크 키를 추출하는 헬퍼
function getNetKey(req) {
  const urlObj = new URL(req.url, 'http://localhost');
  const key = (urlObj.searchParams.get('network') || defaultNetKey).toLowerCase();
  return NETWORKS[key] ? key : defaultNetKey;
}
function getProviderFor(req) { return providers[getNetKey(req)]; }
function getWalletFor(req) { return wallets[getNetKey(req)] || null; }
function getNetFor(req) { return NETWORKS[getNetKey(req)]; }
...
async function handleWalletInfo(req, res) {
  const w = getWalletFor(req);
  const p = getProviderFor(req);
  const n = getNetFor(req);
  if (!w) {
...
async function handleBalance(req, res) {
  ...
  const nonce = await p.getTransactionCount(address);
...
async function handleSendEth(req, res) {
  ...
  try {
    // 송금 전 nonce 조회
    const nonceBefore = await p.getTransactionCount(w.address);
    ...
    // 송금 후 nonce 조회
    const nonceAfter = await p.getTransactionCount(w.address);
    ...
    jsonResponse(res, 200, {
      ...
      explorerLink: n.explorer + '/tx/' + tx.hash,
    });
  } finally {
    isSending = false;
  }
...
async function handleTxTrack(req, res) {
  ...
  if (!tx) {
    jsonResponse(res, 404, { error: '트랜잭션을 찾을 수 없습니다. ' + n.name + ' 테스트넷 트랜잭션인지 확인하세요.' });
    return;
  }
...
async function handleVerifySig(req, res) {
  ...
  jsonResponse(res, 200, {
    ...
    explanation: isValid
      ...
      : '주소 불일치 또는 복원 실패. ' + n.name + ' 테스트넷 트랜잭션인지 확인하세요.',
  });
}

/**
 * 브릿지 TX 분석 — TX가 브릿지 트랜잭션인지 분석
 * hash 쿼리 파라미터 필수, network 선택
 * data 유무, function selector, 브릿지 컨트랙트 여부 등을 분석하여 반환
 */
async function handleBridgeTx(req, res) {
  var p = getProviderFor(req);
  var n = getNetFor(req);
  var netKey = getNetKey(req);
  var urlObj = new URL(req.url, 'http://localhost');
  var txHash = urlObj.searchParams.get('hash');

  if (!txHash) {
    jsonResponse(res, 400, { error: 'hash 파라미터가 필요합니다. 예: /api/bridge-tx?hash=0x...' });
    return;
  }

  var tx = await p.getTransaction(txHash);
  if (!tx) {
    jsonResponse(res, 404, { error: '트랜잭션을 찾을 수 없습니다. ' + n.name + ' 테스트넷 트랜잭션인지 확인하세요.' });
    return;
  }

  var receipt = await p.getTransactionReceipt(txHash);

  // 수수료 계산 (영수증이 있는 경우)
  var gasUsed = null;
  var feeEth = null;
  var gasPriceGwei = null;
  var status = 'pending';
  ...
  var dataLength = hasData ? (txData.length - 2) / 2 : 0; // 바이트 수
  var functionSelector = hasData && txData.length >= 10 ? txData.slice(0, 10) : null;

  // 알려진 브릿지 컨트랙트 주소 (Sepolia 테스트넷 기준)
  var knownBridgeContracts = [
    '0x49048044D57e1C92A77f79988d21Fa8fAF74E97e'.toLowerCase(), // Base Bridge (Sepolia)
    '0xFBb0621E0B23b5478B630BD55a5f21f67730B0F1'.toLowerCase(), // Optimism Bridge (Sepolia)
    '0xFF00000000000000000000000000000000000420'.toLowerCase(), // OP Portal
    '0x4200000000000000000000000000000000000010'.toLowerCase(), // L2 Standard Bridge
    '0x4200000000000000000000000000000000000007'.toLowerCase(), // L2 Cross Domain Messenger
  ];

  var toAddress = tx.to ? tx.to.toLowerCase() : '';
  var isBridgeContract = knownBridgeContracts.indexOf(toAddress) !== -1;
  var isBridgeTx = hasData && (isBridgeContract || dataLength > 32);

  // 네트워크 레이어 판별 (networks.js의 layer 필드 사용)
  var networkLayer = n.layer || 'L1';
  ...
  if (isBridgeTx) {
    if (networkLayer === 'L1') {
      direction = 'L1 → L2 (입금)';
      explanation = 'L1(' + n.name + ')에서 L2로 자산을 브릿지하는 트랜잭션입니다. ' +
        'data 필드에 브릿지 컨트랙트 호출 데이터가 포함되어 있습니다.';
    } else {
      direction = 'L2 → L1 (출금)';
      explanation = 'L2(' + n.name + ')에서 L1으로 자산을 인출하는 트랜잭션입니다. ' +
        '출금 증명(withdrawal proof) 과정이 필요할 수 있습니다.';
    }
    if (isBridgeContract) {
      explanation += ' 알려진 브릿지 컨트랙트(' + tx.to + ')로의 호출이 감지되었습니다.';
    }
  } else {
    direction = '해당 없음';
    explanation = '일반 전송 트랜잭션으로 브릿지 트랜잭션이 아닌 것으로 판단됩니다.';
    if (!hasData) {
      explanation += ' data 필드가 비어 있어 단순 ETH 전송입니다.';
    }
  }
...
const server = http.createServer(function (req, res) {
  ...
  // API 라우트: /api/network-info (프론트엔드용 네트워크 설정)
  if (urlPath === '/api/network-info' && req.method === 'GET') {
    jsonResponse(res, 200, {
      default: defaultNetKey,
  ...
  // API 라우트: /api/bridge-tx?hash=0x...&network=giwa
  if (urlPath === '/api/bridge-tx' && req.method === 'GET') {
    handleBridgeTx(req, res).catch(function (err) {
      jsonResponse(res, 502, { error: err.message });
    });
    return;
  }
...
server.listen(PORT, function () {
  console.log('');
  console.log('  ============================================');
  console.log('   Blockchain Lab #3 —', NET.name, '실습 서버');
  console.log('  ============================================');
  console.log('');
  console.log('   URL     : http://localhost:' + PORT);
  console.log('   네트워크 :', NET.name, '(' + NET.description + ')');
  console.log('   RPC     :', NET.rpcUrl);

  // 지갑 주소 출력 (없으면 경고)
  if (wallets[defaultNetKey]) {
    console.log('   지갑    :', wallets[defaultNetKey].address);
  } else {
    console.log('   지갑    : ⚠️  PRIVATE_KEY 없음 — 읽기 전용 모드');
    console.log('            (send-eth, wallet-info API 사용 불가)');
  }
  console.log('   지원    :', Object.keys(NETWORKS).join(', '), '(UI에서 전환 가능)');

  console.log('');
  console.log('   [API 엔드포인트]');
  console.log('   GET  /api/network-info        현재 네트워크 설정');
  ...
  console.log('   GET  /api/bridge-tx?hash=        브릿지 TX 분석');
  console.log('');
